...
#####################################
raster("D:/HYCOM_NEP/hycom_combine_2012-01-01.grd")
envdir<-"D:/HYCOM_NEP_2"

# start and end dates at daily resolution
st_date<-as.Date("2012-01-01")
en_date<-as.Date("2012-01-01")
...
for (i in 1:length(udates)){
  ...
  myURL <- try(HMMoce:::get.hycom(limits=c(-180.04, -109, 17.96, 61.08), time=udates[i],
...
source(here("scripts", "functions","calculate_eke.r"))
source(here("scripts", "functions","calculate_ild.r"))
source(here("scripts", "functions","calculate_n2.r"))

# 6) SST_SD

# get a list of downloaded sst files
sstList <- list.files(path = "D:/HYCOM_NEP_2") %>% 
...
water_temp_files<-list.files(path = "D:/HYCOM_NEP_2") %>% 
  grep(pattern = "temp", value = TRUE)


salinity_files<-list.files(path = "D:/HYCOM_NEP_2") %>% 
  grep(pattern = "sal", value = TRUE)

# get depth var indices
nc <- "D:/HYCOM_NEP_2/hycom_temp_2012-01-01.nc" %>% 
  RNetCDF::open.nc()
...
for (i in 1:length(water_temp_files)){
  ...
  wtemp <- paste0("D:/HYCOM_NEP_2/", water_temp_files[i]) %>% 
    raster::brick(varname = 'water_temp') #* scale + offset
  #calcualte ild
  ild.5 <- calc(wtemp, calculate_ild)
  writeRaster(ild.5, paste0("D:/HYCOM_NEP_2/", "hycom_ild5", 
